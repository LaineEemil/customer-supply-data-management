    ...
    if (!empty($rivi)) {
        ...
        while($rivi) {
            ...
            if (!empty($rivi_l["laskutunnus"])) {
                ...
                echo "<li>";
                echo "<form action=tuntityolasku_luo.php?tyokohde=$rivi[kohdetunnus] method=post>". "Asiakkaan nimi: " . str_replace("”","ö",str_replace("„","ä", $rivi_a["nimi"])). " | Kohteen osoite: " . 
                str_replace("„","ä", $rivi["osoite"]) . " | Kohteen kuvaus: " . str_replace("”","ö",str_replace("„","ä", $rivi["kuvaus"])) . " | <input type=submit id=nappi value=".'"'."Luo työlasku".'"'.">"."</form action>"."<br>";
                echo "</li>";
                echo "</ul>";
                $rivi = $kysely->fetch();
            }
