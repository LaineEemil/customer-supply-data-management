...
require 'yhteys.php';
    ...
    echo "<h1> Lisää kohteelle työtunteja </h1>";
    if (!empty($rivi)) {
        while($rivi) {
            ...
            echo "<li>";
            echo "<form action=lisaa_tyotunnit.php?tyokohde=$rivi[kohdetunnus] method=post>". "Asiakkaan nimi: " .str_replace("”","ä",str_replace("„","ä",$rivi_a["nimi"])). " | Kohteen osoite: " . 
            str_replace("”","ä",str_replace("„","ä",$rivi["osoite"])). " | Kohteen kuvaus: " . str_replace("”","ä",str_replace("„","ä",$rivi["kuvaus"])). " | <input type=submit id=nappi value=Valitse>"."</form action>"."<br>";
            echo "</li>";
            echo "</ul>";
            $rivi = $kysely->fetch();
        }
