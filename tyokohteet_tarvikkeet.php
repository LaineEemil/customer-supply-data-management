...
require 'yhteys.php';
    ...
    echo "<h1> Lisää kohteessa käytetyt työtarvikkeet </h1>";
    if (!empty($rivi)) {
        while($rivi) {
            ...
            echo "<li>";
            echo "<form action=valitsetuotteet.php?tyokohde=$rivi[kohdetunnus] method=post>". "Asiakkaan nimi: " . str_replace("„","ä", $rivi_a["nimi"]). " | Kohteen osoite: " . 
            str_replace("„","ä", $rivi["osoite"]) . " | Kohteen kuvaus: " . str_replace("”","ö",str_replace("„","ä", $rivi["kuvaus"])) . " | <input type=submit id=nappi value=Valitse>"."</form action>"."<br>";
            echo "</li>";
            echo "</ul>";
            $rivi = $kysely->fetch();
        }
